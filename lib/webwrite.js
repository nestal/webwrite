...
function InitViewer( )
{
	...
	$("#prop").hide() ;
	$("#prop_button").off() ;
	$("#prop_button").click( function(){ Load("?prop");} ) ;
}
...
function InitProperties( meta )
{
	var real_href	= document.location.toString() ;
	var href		= real_href.substr(0, real_href.indexOf('?') ) ;
	
	// initialize name and the link
	$("#meta_name").html(meta.name) ;
	...
	// show mime type
	$("#meta_type").html(meta.type) ;

	// show modified time		
	var modified = new Date() ;
	modified.setTime(meta["last-modified"] * 1000) ;
