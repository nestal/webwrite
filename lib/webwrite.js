...
function SaveDoc()
{
	...
	if ( editor != null )
	{
	    ...
	    doc.innerHTML = $dom.html();
		var xml_str = new XMLSerializer().serializeToString( doc );
...
//				$("#viewer").html( TransformDocument(data) ) ;
			}
		);
	}
}
...
function TransformDocument( data )
{
	...
	$dom.find("a").click( function(ev){ OnClickLinks( ev, this ); }) ;
	...
	$dom.find("span.edit_cmd").click(ToggleEdit);
