...
function main()
{
	...
	$.getJSON( "?var", function(data){
		...
		$(".logo").click( function(){
			var href = data.wb_root + "/" + data.main ;
...
function ToRelativeHref( a, wb_base )
{
	var href = a.attr("href") ;
	var abs_re = /^(https?:\/\/|mailto:).*/ ;
...
function OnClickLinks( ev, a, wb_base )
{
	...
	if ( IsValidRelativeHref( href ) )
	{
		...
		// push the new href to history before calling LoadViewer().
...
function LoadViewer( wb_base )
{
	// the URL is always relative the the current document location
	// if you want a new URL, just change the current document location (i.e. the
	// one in the address bar) by calling history.pushState()
	$("#content").load( "?lib=viewer.html", function()
	{
		$("#viewer").load( "?load", function()
		{
			...
			$("a", "#viewer").click( function(ev){ OnClickLinks( ev, this, wb_base ); }) ;
		} ) ;
...
function InitToolbar( editor, wb_base )
{
	$("#save_button").click( function()
	{
		// do some transform before uploading to server
		var $dom = $( "<div>" + editor.getValue() + "<div>" );
		$dom.find( "a" ).each( function(){ ToRelativeHref( $(this), wb_base ); }) ;
